...
# nvolve_hash
RUN git clone https://github.com/nvolprotocol/nvolve_hash
RUN cd nvolve_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} nvolve
RUN useradd -u ${USER_ID} -g nvolve -s /bin/bash -m -d /nvolve nvolve
...
RUN mkdir /nvolve-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /nvolve-src && \
