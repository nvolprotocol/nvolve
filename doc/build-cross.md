Cross-compiliation of Nvolve Core
===============================

Nvolve Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Nvolve.
...
Note that for WSL the Nvolve Core source path MUST be somewhere in the default mount file system, for
example /usr/src/nvolve, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Nvolve Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Nvolve Core source code).
