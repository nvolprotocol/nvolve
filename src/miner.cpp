...
void static NvolveMiner(const CChainParams& chainparams)
{
    LogPrintf("NvolveMiner -- started\n");
    return;
}

int GenerateNvolves(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&NvolveMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
