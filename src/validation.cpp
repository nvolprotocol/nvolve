...
#if defined(NDEBUG)
# error "Nvolve Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Nvolve
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        ...
        nSubsidy = 0.0500;        
    } else if (nPrevHeight <= 27750 ) {
        nSubsidy = 1.4000;        
    } else if (nPrevHeight <= 83250 ) {
        nSubsidy = 0.9800;        
    } else if (nPrevHeight <= 262500 ) {
        nSubsidy = 0.6860;        
    } else if (nPrevHeight <= 525000 ) {
        nSubsidy = 0.4802;        
    } else if (nPrevHeight <= 787500 ) {
        nSubsidy = 0.3361;        
    } else if (nPrevHeight <= 1387500 ) {
        nSubsidy = 0.2353;
    } else if (nPrevHeight <= 1942650 ) {
        nSubsidy = 0.1647;
    } else if (nPrevHeight <= 2175000 ) {
        nSubsidy = 0.1153;        
    } else if (nPrevHeight <= 2415000 ) {
        nSubsidy = 0.0807;        
    } else if (nPrevHeight <= 2655000 ) {
        nSubsidy = 0.0565;        
    } else if (nPrevHeight <= 2895000 ) {
        nSubsidy = 0.0395;        
    } else if (nPrevHeight <= 3135000 ) {
        nSubsidy = 0.0277;        
    } else if (nPrevHeight <= 3375000 ) {
        nSubsidy = 0.0194;        
    } else if (nPrevHeight <= 3615000 ) {
        nSubsidy = 0.0136;
    } else if (nPrevHeight <= 3855000 ) {
        nSubsidy = 0.0095;
    } else if (nPrevHeight <= 4095000 ) {
        nSubsidy = 0.0066;        
    } else if (nPrevHeight <= 4335000 ) {
        nSubsidy = 0.0047;        
    } else if (nPrevHeight <= 4575000 ) {
        nSubsidy = 0.0033;        
    } else if (nPrevHeight <= 4815000 ) {
        nSubsidy = 0.0023;        
    } else if (nPrevHeight <= 5055000 ) {
        nSubsidy = 0.0016;        
    } else if (nPrevHeight <= 5295000 ) {
        nSubsidy = 0.0011;        
    } else if (nPrevHeight <= 5535000 ) {
        nSubsidy = 0.0008;
    } else if (nPrevHeight <= 5775000 ) {
        nSubsidy = 0.0005;
    } else {
        nSubsidy = 0.0002;
    }
...
static int64_t nTimeNvolveSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(NVOLVE): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // NVOLVE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(NVOLVE): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(NVOLVE): spork is off, skipping transaction locking checks\n");
    }
    ...
    // NVOLVE : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(NVOLVE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(NVOLVE): couldn't find smartnode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeNvolveSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Nvolve specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeNvolveSpecific * MICRO, nTimeNvolveSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(NVOL): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
