...
const char * const BITCOIN_CONF_FILENAME = "nvolve.conf";
const char * const BITCOIN_PID_FILENAME = "nvolved.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nvolved -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NvolveCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NvolveCore
    // Mac: ~/Library/Application Support/NvolveCore
    // Unix: ~/.nvolvecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NvolveCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NvolveCore";
#else
    // Unix
    return pathRet / ".nvolvecore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty nvolve.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Nvolve Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Nvolve Core developers";
    }
