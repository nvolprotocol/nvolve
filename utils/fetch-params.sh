...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/NvolveParams"
else
    PARAMS_DIR="$HOME/.nvolve-params"
fi
...
DOWNLOAD_URL="https://github.com/nvolprotocol/nvolve/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with NVOL_DISABLE_SOMETHING=1
NVOL_DISABLE_WGET="${NVOL_DISABLE_WGET:-}"
#NVOL_DISABLE_IPFS="${NVOL_DISABLE_IPFS:-}"
NVOL_DISABLE_CURL="${NVOL_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$NVOL_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$NVOL_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$NVOL_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Nvolve parameters!
...
main() {
    ...
    cat <<EOF
NVOL - fetch-params.sh

This script will fetch the Nvolve SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Nvolve SNARK parameters. Note that it is
